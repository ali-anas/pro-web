...
function SingleProject({ project }) {
  return (
    <div className="col col--6 mb-8">
      <div
        className={clsx(
          "w-full h-content rounded-md md:rounded-xl shadow-small hover:shadow-hover overflow-hidden",
        )}
      >
        ...
        <div className="flex flex-col flex-nowrap justify-between h-content p-4 bg-bgSurface">
          <h3 className="text-center w-full mb-1 text-textPrimary">
            {project.title}
          </h3>
          <p className="mb-2 line-clamp-2 text-textSecondary">
            {project.intro}
          </p>
          <div className="flex flex-nowrap flex-row justify-start items-center">
            {project.github_url && (
              <HrefElem to={project.github_url} title="Github" />
            )}
            {project.youtube_url && (
              <HrefElem to={project.youtube_url} title="YouTube" />
            )}
            {project.live_url && (
              <HrefElem to={project.live_url} title="Live" />
            )}
          </div>
        </div>
      </div>
